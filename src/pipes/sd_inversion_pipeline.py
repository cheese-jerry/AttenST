...
from src.SPI_inversion import inversion_step
...
class SDDDIMPipeline(StableDiffusionImg2ImgPipeline):
    ...
    def __call__(
        ...
        timesteps, num_inversion_steps = self.get_timesteps(num_inversion_steps, strength,
                                                            device)  # num_inversion_steps = 50  timesteps=[981,961,.....21.1]
        latent_timestep = timesteps[:1].repeat(batch_size * num_images_per_prompt)  # 981
        ...
        # 8. Denoising loop
        num_warmup_steps = len(timesteps) - num_inversion_steps * self.scheduler.order  # num_warmup_steps=0
        ...
        with self.progress_bar(total=num_inversion_steps) as progress_bar:
            for i, t in enumerate(reversed(timesteps)):  # i=0,t=1-->i=1,t=21
